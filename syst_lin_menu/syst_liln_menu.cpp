...
Menu::Menu(){
    ifstream menu_f("menu.txt", ios::in);

    option = newVect<string> (3);

    string tmp="";
    menu_f >> choices;
    getline(menu_f, tmp);
    for(size_t i(0); i<choices; i++) {
        getline(menu_f, option[i]);
    }
...
/// Calculs et résultats
    ...
    /* CALCUL PAR LA METHODE DE GAUSS */
        case 1:
            cout << "Résolution par la méthode de Gauss :" << endl;
            solver->gaussElim();
    ...
    /* CALCUL PAR LA METHODE DE CHOLESKY */
        case 2:
            cout << "Résolution par la méthode de Cholesky :" << endl;
